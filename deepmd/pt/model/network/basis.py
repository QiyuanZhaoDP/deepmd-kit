        ...
        *,
        ...
        if learnable:
            self.frequencies = torch.nn.Parameter(
                data=torch.arange(1, order + 1, dtype=self.prec, device=env.DEVICE),
                requires_grad=True,
            )
        else:
            self.register_buffer(
                "frequencies",
                torch.arange(1, order + 1, dtype=self.prec, device=env.DEVICE),
            )
        ...
        *,
        ...
        if learnable:
            ...
            self.register_buffer(
                ...
                np.pi
                * torch.arange(
                    1, self.num_radial + 1, dtype=torch.float, device=env.DEVICE
                ),
            )
...
class GaussianExpansion(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "gaussian_centers",
            torch.arange(min, max + step, step, dtype=torch.float, device=env.DEVICE),
        )
